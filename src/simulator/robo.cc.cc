...
bool robo::calc_forca() {
	...
	while(aux_ang >= 2 * M_PI)
		aux_ang -= 2 * M_PI;
	while(aux_ang < 0)
		aux_ang += 2 * M_PI;

	if(STRATEGY_TO_SIMULATOR_VOLT) {
	    nova_f_roda_esquerda = v_desejada_roda_esquerda;
	    nova_f_roda_direita = v_desejada_roda_direita;
	} else {
	...
	double forca_maxima_esquerda = ((ROBO_REDUCAO * ROBO_MOTOR_RENDIMENTO * ROBO_MOTOR_RESISTENCIA_TORQUE * ROBO_V_MAX) / (ROBO_RAIO_RODA * ROBO_MOTOR_RESISTENCIA_ELETRICA));// - (ROBO_MOTOR_RESISTENCIA_ROTACAO * (v_roda_esquerda / ROBO_RAIO_RODA) / ROBO_RAIO_RODA) - ((ROBO_MOMENTO_INERCIA_2 * aceleracao_angular_roda_esquerda) / ROBO_RAIO_RODA);
	double forca_maxima_direita = ((ROBO_REDUCAO * ROBO_MOTOR_RENDIMENTO * ROBO_MOTOR_RESISTENCIA_TORQUE * ROBO_V_MAX) / (ROBO_RAIO_RODA * ROBO_MOTOR_RESISTENCIA_ELETRICA));// - (ROBO_MOTOR_RESISTENCIA_ROTACAO * (v_roda_direita / ROBO_RAIO_RODA) / ROBO_RAIO_RODA) - ((ROBO_MOMENTO_INERCIA_2 * aceleracao_angular_roda_direita) / ROBO_RAIO_RODA);

	/*FILE *fd = fopen("forca_log.txt", "a");

	fprintf(fd, "n_f_e %f. max_f_e: %f. n_f_d: %f. max_f_d: %f.\n", nova_f_roda_esquerda, (float) forca_maxima_esquerda, nova_f_roda_direita, (float) forca_maxima_direita);
	fprintf(fd, "a_e: %f. a_a_e: %f. a_d: %f. a_a_d: %f.\n", aceleracao_roda_esquerda, aceleracao_angular_roda_esquerda, aceleracao_roda_direita, aceleracao_angular_roda_direita);
	fclose(fd);*/

	if(nova_f_roda_esquerda > forca_maxima_esquerda)
	    nova_f_roda_esquerda = (float) forca_maxima_esquerda;
	if(nova_f_roda_esquerda < -forca_maxima_esquerda)
	    nova_f_roda_esquerda = (float) -forca_maxima_esquerda;

	if(nova_f_roda_direita > forca_maxima_direita)
	    nova_f_roda_direita = (float) forca_maxima_direita;
	if(nova_f_roda_direita < -forca_maxima_direita)
	    nova_f_roda_direita = (float) -forca_maxima_direita;
	...
	// Adicionando força contrária ao movimento do robô.
	float f_contraria_movimento = velocidade->getModulo() * ROBO_K_ATRITO_ENGRENAGENS;
	*nova_forca_resultante += *(new vetor2d(- f_contraria_movimento * cos(velocidade->getAng()), - f_contraria_movimento * sin(velocidade->getAng())));
	...
	*nova_forca_resultante += f_atrito_rodas;
	...
	FILE *fd = fopen("arq", "a");
	fprintf(fd, "%f\n", nova_forca_resultante->getModulo());
	fclose(fd);
...
bool robo::calc_atributos(campo *c, float x_bola, float y_bola, vetor2d v_bola, vetor2d f_bola, robo *time1[N_JOGADORES_POR_TIME - 1], robo *time2[N_JOGADORES_POR_TIME]) {
	...
	for(int i = 0; i < N_JOGADORES_POR_TIME - 1; i++) {
		colisao_robo_robo(time1[i]);
		colisao_robo_robo(time2[i]);
	}
	
	colisao_robo_robo(time2[N_JOGADORES_POR_TIME - 1]);

	colisao_robo_campo(c);
	colisao_robo_bola(x_bola, y_bola, v_bola, f_bola);
