...
float dist_overlap_field_robot(float cx[16], float cy[16], float ra_x, float ra_y, float ra_a){
    ...
    //se encontra totalmente dentro do campo
    if(ra_x2<=cx2[8]+(cx2[9]-cx2[8])/2-diagp2&&b_y2<=cy2[9]+(cx2[8]-cx2[9])/2-diagp2)
        return(0);
    if(ra_x2<=cx2[11]-diagp2&&b_y2<=cy2[10]-diagp2)
        return(0);
    
    //armazenando as coordenadas dos vertices do robo
    for(i=0;i<4;i++){
        vertx[i]=ra_x2+diagp2*cos(ra_a2+PI/4+i*PI/2);
        verty[i]=ra_y2+diagp2*sin(ra_a2+PI/4+i*PI/2);
    }
    ...
    for(i=0;i<4;i++){
        if(verty[i]>=cy2[8]){
            ...
            if(px<=cx2[8])if(dist<sqrt((verty[i]-py)*(verty[i]-py)+(vertx[i]-px)*(vertx[i]-px))
                               dist=sqrt((verty[i]-py)*(verty[i]-py)+(vertx[i]-px)*(vertx[i]-px));
        }
        if(vertx[i]>=cx2[11]){
            px=cx2[11];
            py=px*(verty[i]/vertx[i]);
            if(py<=cy2[11])if(dist<sqrt((verty[i]-py)*(verty[i]-py)+(vertx[i]-px)*(vertx[i]-px))
                               dist=sqrt((verty[i]-py)*(verty[i]-py)+(vertx[i]-px)*(vertx[i]-px));
        }
        if(verty[i]>cy2[9]){
            px=(cy2[8]-tg*cx2[8])/(verty[i]/vertx[i]-tg);
            if(px>cx2[8]&&px<cx2[9]&&px<=vertx[i]){
                py=px*(verty[i]/vertx[i]);
