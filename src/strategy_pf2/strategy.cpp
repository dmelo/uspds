...
void FollowBall(Environment *env, Robot *robot)
{
	...
	#ifdef BLUE
	//novo ponto de atraçao (px,py)
	py = dy + env->currentBall.pos.y;
	px = dx + env->currentBall.pos.x;
	#else
	//novo ponto de atraçao (px,py)
	py = env->currentBall.pos.y - dy;
	px = env->currentBall.pos.x - dx;		
	#endif
	...
	if(robot->pos.x > env->currentBall.pos.x || rb > 0.364) //20 graus
#endif
	if((py > FBOT && py < FTOP)&&((cp->get_team_number() == 1 && (robot->pos.x> env->currentBall.pos.x || rb > 0.364))||(cp->get_team_number() == 2 && (robot->pos.x > env->currentBall.pos.x || rb > 0.364)))) //se o ponto está dentro do campo
	{
...
int main(int argc, char **argv) {
	...
	if(cp->get_team_number() == 2) {
		Defesa(env,env->home+0, (int) FRIGHTX - 1.5);
		Defesa(env,env->home+1, (int) FRIGHTX - 13);
	}
	else {
		Defesa(env,env->home+0, FLEFTX + 1.5);
		Defesa(env,env->home+1, FLEFTX + 13);
	}
	//printf("\n\n");
	for(int i = 0; i < N_JOGADORES_POR_TIME; i++) {
